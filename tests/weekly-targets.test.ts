...
describe("Weekly Sync 대상 — 완료 후 보고 유예기간", () => {
  const tickets = [
    ...
    { key: "TM-CANCELLED", status: "철회/반려/취소" },
    ...
    { key: "TM-DONE-UPDATED", status: "개발완료", updatedAt: "2026-08-06T00:00:00.000Z" },
  ...
  it("숨김 과제는 상태와 무관하게 제외", () => {
    ...
    assert.deepEqual(result.targets.map(t => t.key), ["TM-DONE-BOUNDARY"]);
